...
class gru_cell:
    ...
    def backward_h_prev(self, step_size, x, r, z, h_prev, h_candidate, h_next, d_loss_d_h_next):
        ...
        print(f"sigmoid_input_z shape: {sigmoid_input_z.shape} (expected size: ({self.hidden_size}, 1))")
        d_z_d_sigmoid_input = sigmoid.derivative(sigmoid_input_z)
        print(f"d_z_d_sigmoid_input shape: {d_z_d_sigmoid_input.shape} (expected size: ({self.hidden_size}, 1))")
        d_sigmoid_input_d_h_prev = self.U_z
        print(f"d_sigmoid_input_d_h_prev shape: {d_sigmoid_input_d_h_prev.shape} (expected size: ({self.hidden_size}, {self.hidden_size}))")
        d_z_d_h_prev = d_z_d_sigmoid_input * d_sigmoid_input_d_h_prev
        print(f"d_z_d_h_prev shape: {d_z_d_h_prev.shape} (expected size: ({self.hidden_size}, {self.hidden_size}))")

        tanh_input = np.matmul(self.W, x) + r * np.matmul(self.U, h_prev) + self.bias_h
        print(f"tanh_input shape: {tanh_input.shape} (expected size: ({self.hidden_size}, 1))")
        d_h_candidate_d_tanh = tanh.derivative(tanh_input) #d(tanh)
        print(f"d_h_candidate_d_tanh shape: {d_h_candidate_d_tanh.shape} (expected size: ({self.hidden_size}, 1))")
        sigmoid_input_r = np.matmul(self.W_r, x) + np.matmul(self.U_r, h_prev) + self.bias_r
        print(f"sigmoid_input_r shape: {sigmoid_input_r.shape} (expected size: ({self.hidden_size}, 1))")
        sigmoid_derivative = sigmoid.derivative(sigmoid_input_r)
        print(f"sigmoid_derivative shape: {sigmoid_derivative.shape} (expected size: ({self.hidden_size}, 1))")
        reset_gate_deriv_side = np.matmul(sigmoid_derivative*self.U_r, np.matmul(self.U, h_prev))
        print(f"reset_gate_deriv_side shape: {reset_gate_deriv_side.shape} (expected size: ({self.hidden_size}, {self.hidden_size}))")
        h_prev_derive_side = (r*self.U)*1
        print(f"h_prev_derive_side shape: {h_prev_derive_side.shape} (expected size: ({self.hidden_size}, {self.hidden_size}))")

        d_h_candidate_d_h_prev = d_h_candidate_d_tanh*(reset_gate_deriv_side + h_prev_derive_side)
        print(f"d_h_candidate_d_h_prev shape: {d_h_candidate_d_h_prev.shape} (expected size: ({self.hidden_size}, {self.hidden_size}))")

        d_h_next_d_h_prev = (
            z*1 + 
            h_prev*d_z_d_h_prev + 
            h_candidate*(-d_z_d_h_prev) + 
            (1-z)*d_h_candidate_d_h_prev
        )
        print(f"d_h_next_d_h_prev shape: {d_h_next_d_h_prev.shape} (expected size: ({self.hidden_size}, {self.hidden_size}))")
        d_loss_d_h_prev = d_loss_d_h_next * d_h_next_d_h_prev
        print(f"d_loss_d_h_prev shape: {d_loss_d_h_prev.shape} (expected size: ({self.hidden_size}, 1))")
    ...
    def backward_combined(self, step_size, x, r, z, h_prev, h_candidate, h_next, d_loss_d_h_next):
        ...
        results_dict = {
            ...
            'd_loss_d_h_prev': d_loss_d_h_prev
        }

        return d_loss_d_h_prev, results_dict
...
def train_gru(gru_cell, series, epochs, learning_rate, sequence_length):
    ...
    for epoch in range(epochs):
        total_loss = 0
        results_dict_total = {
            'd_loss_dW': np.zeros_like(gru_cell.W),
            'd_loss_dU': np.zeros_like(gru_cell.U),
            'd_loss_d_bias_h': np.zeros_like(gru_cell.bias_h),
            'd_loss_dW_z': np.zeros_like(gru_cell.W_z),
            'd_loss_dU_z': np.zeros_like(gru_cell.U_z),
            'd_loss_d_bias_z': np.zeros_like(gru_cell.bias_z),
            'd_loss_dW_r': np.zeros_like(gru_cell.W_r),
            'd_loss_dU_r': np.zeros_like(gru_cell.U_r),
            'd_loss_d_bias_r': np.zeros_like(gru_cell.bias_r),
            'd_loss_d_h_prev': np.zeros_like(h_prev)
        }
        ...
        for t in range(len(series) - 1):
            ...
            d_loss_d_h_prev, results_dict_iter = gru_cell.backward_combined(learning_rate, x, r, z, h_prev, h_candidate, h_next, d_loss_d_h_next)

            # Accumulate gradients

            '''print("Before updating results_dict_total:")
            print("results_dict_total:")
            for key, value in results_dict_total.items():
                print(f"{key}: shape {value.shape}")

            print("results_dict_iter:")
            for key, value in results_dict_iter.items():
                print(f"{key}: shape {value.shape}")'''

            for key in results_dict_total:
                results_dict_total[key] += results_dict_iter[key]

            steps_in_sequence += 1

            # If we've reached the end of a sequence, update weights
            if steps_in_sequence == sequence_length:
                gru_cell.update_weights(results_dict_total, learning_rate)
                # Reset for the next sequence
                results_dict_total = {key: np.zeros_like(value) for key, value in results_dict_total.items()}
                steps_in_sequence = 0
...
'''def train_gru(gru_cell, series, epochs, learning_rate, sequence_length):
    ...
    for epoch in range(epochs):
        total_loss = 0
        results_dict_total = np.zeros(10)
        steps_in_sequence = 0  # Counter to track steps within the sequence
        ...
        for t in range(len(series) - 1):
            ...
            # Accumulate gradients
            results_dict_total += results_dict_iter
            steps_in_sequence += 1

            # If we've reached the end of a sequence, update weights
            if steps_in_sequence == sequence_length:
                gru_cell.update_weights(results_dict_total, learning_rate)
                results_dict_total = np.zeros(10)  # Reset for the next sequence
                steps_in_sequence = 0
